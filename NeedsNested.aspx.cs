...
{
    static int total = File.ReadAllLines(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "needsNested.txt")).Count() + 3;
    ...
    {
        var make = File.Create(Path.Combine(AppDomain.CurrentDomain.BaseDirectory,"needsNested.txt"));
        make.Close();
    ...
    protected void inProgress(string[,] edited)
    {
        ...
        {
            case 1:
                var file1 = File.Create(Path.Combine(AppDomain.CurrentDomain.BaseDirectory,"P1.txt"));
                file1.Close();
                using (var sw = new StreamWriter(Path.Combine(AppDomain.CurrentDomain.BaseDirectory,"P1.txt"), true))
                {
                    for (int i = 0; i < total; i++)
                    {
                        string output = "";
                ...
                break;
            case 2:
                var file2 = File.Create(Path.Combine(AppDomain.CurrentDomain.BaseDirectory,"P2.txt"));
                file2.Close();
                using (var sw = new StreamWriter(Path.Combine(AppDomain.CurrentDomain.BaseDirectory,"P2.txt"), true))
                {
                    for (int i = 0; i < total; i++)
                    {
                        string output = "";
                ...
                break;
            case 3:
                var file3 =File.Create(Path.Combine(AppDomain.CurrentDomain.BaseDirectory,"P3.txt"));
                file3.Close();
                using (var sw = new StreamWriter(Path.Combine(AppDomain.CurrentDomain.BaseDirectory,"P3.txt"), true))
                {
                    for (int i = 0; i < total; i++)
                    {
                        string output = "";
