...
public partial class Contact : System.Web.UI.Page
{
    ...
    protected string[,] ReadFile()
    {
        ...
        string[] split = new string[1300];
        var file = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "P3.txt");
        if (File.Exists(file))
        {

            using (StreamReader SR = new StreamReader(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "P3.txt")))
            {
                string line;
                int m = 0;
                for (int i = 0; i < 50; i++)
    ...
    private void GoBack()
    {
        File.Delete(Path.Combine(AppDomain.CurrentDomain.BaseDirectory,"P3.txt"));
